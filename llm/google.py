...
Запрос пользователя: {prompt}"""
            ...
            if conversation_history:
                logger.info(
                    f"Processing conversation history: {len(conversation_history)} messages")
                # Используем всю доступную историю для контекста
                ...
                logger.info(
                    f"Converted to Gemini format: {len(gemini_history)} messages")
            else:
            ...
            response = await convo.send_message(enhanced_prompt)
            logger.info(
            ...
            if (
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to convert args, using dict(): {e}")
                    arguments = dict(tool_call_data.args)
            ...
            try:
                ...
                if "function_call" in str(e):
                    logger.warning(
                        "Response contains function_call but was not processed correctly")
                    return "Извините, произошла ошибка при обработке запроса. Попробуйте переформулировать вопрос."
                else:
                    logger.error(
                        f"Unexpected error getting response text: {e}")
    ...
    async def generate_response_with_audio(
        ...
        try:
            ...
            mime_type = {
                ...
                '.flac': 'audio/flac',
                '.m4a': 'audio/m4a'
                # По умолчанию ogg для голосовых сообщений Telegram
            }.get(file_extension, 'audio/ogg')
            ...
            logger.error(
                f"Ошибка при обработке аудио: {str(e)}", exc_info=True)
    ...
    async def generate_audio_from_text(
        ...
        try:
            ...
            logger.error(
                f"Ошибка при генерации аудио: {str(e)}", exc_info=True)
